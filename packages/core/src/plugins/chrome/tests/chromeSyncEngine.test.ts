...
describe('ChromeExtensionSyncEngine', () => {
  ...
  beforeEach(() => {
    ...
    const storageKeyPrefix = '@stores/test-sync:';
    storageAdapter1 = new ChromeStorageAdapter({ area: 'local', storageKeyPrefix });
    storageAdapter2 = new ChromeStorageAdapter({ area: 'local', storageKeyPrefix });
  ...
  it('should handle bidirectional sync with complex state', async () => {
    ...
    const createTodoStore = (storage: ChromeStorageAdapter, syncEngine: ChromeExtensionSyncEngine) =>
      createBaseStore<TodoState>(
        set => ({
          ...
          addTodo: text =>
            set(state => ({
              todos: [...state.todos, { id: `${Date.now()}-${Math.random()}`, text, completed: false }],
            })),
          toggleTodo: id =>
            set(state => ({
