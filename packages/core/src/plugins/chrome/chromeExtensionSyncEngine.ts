...
export class ChromeExtensionSyncEngine implements SyncEngine {
  ...
  private attachListener(): void {
    ...
    if (ENABLE_LOGS) console.log('[📡 SyncEngine 📡] Storage listener attached for sessionId:', this.sessionId);
  }
  ...
  private onStorageChanged = async (changes: Record<string, chrome.storage.StorageChange>, areaName: AreaName): Promise<void> => {
    ...
    for (const registeredStore of this.registrations) {
      ...
      try {
        ...
        if (!persistValue || persistValue.syncMetadata?.origin === this.sessionId) continue;

        if (ENABLE_LOGS) console.log(`[📡 SyncEngine 📡] onStorageChanged('${storeKey}'): Applying remote update`);

        const metadata = persistValue?.syncMetadata;
        const values = this.extractChangedFields(container, persistValue.state, metadata, change.oldValue);
  ...
  private extractChangedFields(
    ...
    if (metadataFields && Object.keys(metadataFields).length > 0) {
      ...
      if (populatedFromMetadata) {
        if (ENABLE_METADATA_LOGS) console.log('[✅ SyncEngine ✅] Extracted field metadata:', Object.keys(metadataFields).join(', '));
        return values;
      }
    }
